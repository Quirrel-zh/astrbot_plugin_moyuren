...
class ConfigManager:
    ...
    def load_config(self) -> Optional[bool]:
        ...
        with open(self.config_file, 'r', encoding='utf-8') as f:
            ...
            if not isinstance(loaded_settings, dict):
                raise ValueError(f"配置文件格式错误：期望字典类型，实际为 {type(loaded_settings)}")
            ...
            for target, settings in loaded_settings.items():
                ...
                self.group_settings[target]['trigger_word'] = settings.get('trigger_word', "摸鱼")
    ...
    def save_config(self) -> Optional[bool]:
        ...
        if not isinstance(self.group_settings, dict):
            raise ValueError(f"保存配置失败：group_settings类型错误 ({type(self.group_settings)})")
